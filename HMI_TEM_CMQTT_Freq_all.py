...
class PPLForm(wx.Frame):
    ...
    def on_tiempo_refresco(self, event):                            # Cuenta el tiempo para los perfiles
        ...
        if (self.otravar == 1) and (self.var_fin == 0):
            ...
            # open files to merge:
            merge_files_all.Merge_it(self.fecha_save)       # junta datos de amp y fase con temperatura
    ...
    def on_tiempo_muestreo(self, event):
        ...
        if not self.paused and self.run:
            try:
                ...
                bat_rp = my_data_volts[-1, 3]
                pin1_rp = my_data_volts[-1, 4]
                pin2_rp = my_data_volts[-1, 5]
                pin3_rp = my_data_volts[-1, 6]
                pin4_rp = my_data_volts[-1, 7]
                pin5_rp = my_data_volts[-1, 8]
